...
var dealData = function(keys,results){
	var q = async.queue(function(item,cback){
	    convertMline(item,results,cback);
	  },50);
	  q.drain = function(){
	    console.log('pid:'+process.pid+'分时线处理结束');
	    REDIS_INSTANCE.quit();
	    process.exit(0);
	  };
...
var convertMline = function(code,results,cback){
    ...
    REDIS_INSTANCE.lrange(key,0,-1,function(err,res){
      if(err){
        ...
        res.forEachSync(function(item,index,cb){
          ...
          console.log('pid:'+key+'over.');
          cback();
        });
      }else{
        console.log('pid:'+key+'不存在');
        cback();
      }      
    });
};
